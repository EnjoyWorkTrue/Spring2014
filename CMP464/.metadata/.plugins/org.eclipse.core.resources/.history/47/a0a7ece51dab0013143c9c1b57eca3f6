...
public class MyPullParser {
		...
		while (eventType != XmlPullParser.END_DOCUMENT) {
			if(eventType == XmlPullParser.START_DOCUMENT) {
				...
				if(xpp.getName().equals("item")){
					...
					Log.i("MyPullParser--",currentItem.title);
				}else if(xpp.getName().equals("description") && currentItem != null){
					//set description for the current item
					Log.e("MyPullParser", xpp.getName());
					currentItem.description = xpp.nextText();
					append_des(currentItem.description);
					Log.i("MyPullParser",currentItem.description);
					Log.e("MyPullParser-----", "finish");
				}
	...
	public String[] getDescription(){
		String[] prep = new String[start1];
		for(int i=0; i<start1;i++){
			prep[i] = description[i];
		}
		return prep;
	}
	@Override
	...
	public String toString1(){
		...
		//	prettyPrint = listOfTitle[i] +","+ prettyPrint;
		//}
		prettyPrint = listOfTitle[0] + ", " + listOfTitle[start-1];
		return prettyPrint;
	}
